...
// ----------------------------------------------------------------------------------------------------------------------- Methods
		...
		{	
			...
			*/
							...
							if(j != 0) // if we have more than one item
							{
								...
								if (_row > 1 && _row != passedItems + 1) // if _row is more than 1 and we didn't reach the _row value yet
								{
									...
									if (passedItems == _row - 1 && _firstArrange) // if we are at the last item in this row and this is our first arrangment, because next time on this function call, we have _rows Array ready already
									{
										...
										var k:int;
										for (k = j - (_row - 1); k < j + 1; k++) // we're exactly going to save this row items
										{
							...
							if(j != 0) // if we have more than one item
							{
								...
								if (_row > 1 && _row != passedItems + 1) // if _row is more than 1 and we didn't reach the _row value yet
								{
									...
									if (passedItems == _row - 1 && _firstArrange) // if we are at the last item in this row and this is our first arrangment, because next time on this function call, we have _rows Array ready already
									{
										...
										var m:int;
										for (m = j - (_row - 1); m < j + 1; m++) // we're exactly going to save this row items
										{
			...
			function vCompactCheck($prevRowSamecolumnItem:*, $RowNum:Number, $isNormal:Boolean = true):Number
			{
				...
				{
					var l:int;
					for (l = 0; l < _rows[$RowNum].length; l++) 
					{
			...
			function hCompactCheck($prevRowSamecolumnItem:*, RowNum:Number, $isNormal:Boolean = true):Number
			{
				...
				{
					var n:int;
					for (n = 0; n < _rows[RowNum].length; n++) 
					{
						if (_rows[RowNum][n].content.width > _maxWItemInRow) _maxWItemInRow = _rows[RowNum][n].content.width;
					}
				}
		...
		{
			var i:int;
			var lng:int = _items.length;
			for (i = 0; i < lng; i++ ) // find the item inside the array
			{
		...
		public function removeAll():void
		{
			...
			_items.splice(0);
			_idCount = 0; // we have no item anymore, so _idCount can be reset too
		}
